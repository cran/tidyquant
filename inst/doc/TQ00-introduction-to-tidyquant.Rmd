...
Minimizing the number of functions reduces the learning curve. What we've done is group the core functions into four categories: 
  ...
  3. __Transmute, `tq_transmute()`, and Mutate, `tq_mutate()`, Quantitative Data__: Perform and scale financial calculations completely within the `tidyverse`. These workhorse functions integrate the `xts`, `zoo`, `quantmod`, and `TTR` packages. 
  ...
  5. __Performance analysis, `tq_performance()`, and portfolio aggregation, `tq_portfolio()`__: The `PerformanceAnalytics` integration enables analyzing performance of assets and portfolios. Refer to [Performance Analysis with tidyquant](TQ05-performance-analysis-with-tidyquant.html).
...
## Integrates the Quantitative Analysis Functionality of xts/zoo, quantmod and TTR
...
Refer below for `PerformanceAnalytics` integration.
